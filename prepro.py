...
model=[u'.*1 、故障时间.*故障号码为.*2 、故障现象为.*3 、是否曾欠费停机.*4 、周围人是否使用正常.*5 、故障地点为.*6 、是否个别网站/第三方应用无法使用.*7 、其他为.*',
	...
	u'.*1 、故障开始时间为.*2 、故障现象为.*3 、手机4G上网是否正常(或是否在所有地点均不好用).*4 、多次重启设备是否恢复.*5 、故障地点.*6 、是否个别网站/第三方应用无法使用（选是时填写网站或软件名）.*7 、其他.*',
	u'.*1 、故障开始时间.*2 、故障现象.*3 、手机4G上网是否正常(或是否在所有地点均不好用).*4 、故障地点.*5 、是否个别网站/第三方应用无法使用（选是时填写网站或软件名）.*6 、其他.*',
